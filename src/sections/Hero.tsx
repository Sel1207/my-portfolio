...
// --- UPDATED STAT ITEM WITH HOVER TOOLTIP ---
...
export function Hero() {
  ...
  return (
    <section className="relative min-h-screen flex flex-col justify-center pt-20 pb-16 overflow-hidden bg-background">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center w-full">
          ...
          >
            <motion.div variants={fadeInUp} className="mb-6">
              <motion.div whileHover={{ scale: 1.05 }} className="inline-block">
                <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white dark:bg-slate-800 shadow-lg border border-slate-200 dark:border-slate-700 transition-colors hover:bg-slate-50 dark:hover:bg-slate-700/80 cursor-default">
                  ...
                  {/* EXACT EXTRACTED CODE FROM YOUR REFERENCE */}
            ...
            <motion.div variants={fadeInUp}>
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-7xl font-bold mb-4 flex flex-col items-start">
                ...
                {/* 1:1 REPLICATED GLOWING GRADIENT USING FRAMER MOTION */}
                <motion.span 
            ...
            {/* --- UPDATED STATS GRID WITH DEFINITIONS --- */}
            <motion.div variants={staggerContainer} className="grid grid-cols-2 sm:grid-cols-4 gap-6">
              <StatItem 
                value="3" 
                label="Year Standing" 
                suffix="rd" 
                tooltip="Calculated based on successfully completed academic units that strictly align with the prescribed university curriculum."
              />
              <StatItem 
                value="6" 
                label="President's Lister" 
                suffix="x" 
                tooltip="An elite quarterly award granted exclusively to Dean's Listers ranking at the very top of their respective undergraduate programs."
              />
              <StatItem 
                value="BMEE" 
                label="Program" 
                tooltip="An accelerated joint degree program allowing students to concurrently earn both a Bachelor of Science and a Master of Science in Electrical Engineering."
              />
              <StatItem 
                value="2023" 
                label="Batch" 
                tooltip="Denotes the academic year of matriculation as a first-year undergraduate student into the university."
              />
            </motion.div>
          </motion.div>
          ...
          >
            ...
            >
              ...
              {/* MEDAL WITH SYNCED GRADIENT ANIMATION */}
              <motion.div 
                animate={{ 
                  y: [0, -10, 0],
                  backgroundPosition: ["0% 50%", "100% 50%", "0% 50%"]
      ...
      {/* --- EXTRACTED SCROLL INDICATOR FROM OTHER SITE --- */}
      <motion.div 
        initial={{ opacity: 0 }} 
        animate={{ opacity: 1 }} 
      ...
      <Dialog open={isImageModalOpen} onOpenChange={setIsImageModalOpen}>
        <DialogContent className="max-w-4xl p-0 bg-slate-900 border-slate-800 overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)] z-[100] outline-none border-none">
          ...
          <div className="flex flex-col items-center">
            <img 
              src="/hero-portrait.jpg" 
              alt="Karl Philip Espino - Full Portrait" 
            ...
            <div className="p-6 w-full text-center bg-slate-950 border-t border-slate-800">
              <DialogDescription className="text-sm text-slate-400 italic leading-relaxed">
                Photo taken during the Mapúa University Senior High School Official Pictorial, Batch 2023.
              </DialogDescription>
            </div>
          </div>
        </DialogContent>
